...
def repeat_input(p):
    if p['input'] == '':
        ...
        instruction = f'First repeat the input, then {p["instruction"]}'
        output = f'{p["input"]}\n{p["output"]}'
        return {'instruction': instruction, 'output': output, 'input': p['input']}
...
if __name__ == '__main__':
    ...
    dataset = dataset.map(repeat_input)
    ...
    if args.sample:
        ...
        dataset.to_json(f'data/{dataset_name}_repeat.jsonl', orient='records', lines=True)
