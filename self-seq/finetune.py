...
def encode_alpaca_format(example, tokenizer, max_seq_length, prompter, add_bos=False):
    '''
...
def encode_with_dialogue_format(example, tokenizer, max_seq_length, add_bos=False):
    messages = example['instruction']
...
def main():
    ...
    elif "instruction" in raw_datasets["train"].column_names:
        # is dialogues
        encode_function = partial(
            encode_with_dialogue_format,
            tokenizer=tokenizer,
            max_seq_length=args.max_seq_length,
            add_bos=args.add_bos,
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if (args.do_eval) and (args.eval_steps != "epoch") and (completed_steps % eval_steps == 0):
                    ...
                    logger.info(f"  Eval PPL: {eval_ppl}")
                    if args.with_tracking:
                        accelerator.log(
                            {
                                "eval_ppl": eval_ppl,
        ...
        if args.do_eval and args.eval_steps == "epoch":
            eval_ppl = 0
            for step, batch in enumerate(tqdm(eval_dataloader)):
                with torch.no_grad():
                    loss = model(**batch, use_cache=False).loss
